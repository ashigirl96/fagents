...
from fagents.tools import streaming_mean
...
class Loop(object):
	...
	def __init__(self, logdir, step=None, log=None, report=None, reset=None):
		"""Execute operations in a loop and coordinate loging and checkpoints.
      ...
      Reported mean scores.
		...
		while True:
			...
			summary, mean_score, global_step, steps_made = sess.run(
				phase.op, phase.feed)
			if self._is_every_steps(phase_step, phase.batch, phase.log_every):
	...
	def _is_every_steps(self, phase_step, batch, every):
		...
		if not every:
			return False
		convered_steps = range(phase_step, phase_step + batch)
		return any((step + 1) % every == 0 for step in convered_steps)
